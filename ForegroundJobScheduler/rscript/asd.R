...
ar1_model <- function(train_set, test_set, update_freq=1) {
  ...
  current_end <- 1
  test_percent <- 0.00
  while (current_end <= (nrow(test_set)-1)) {
...
ar_logistic_model <- function(dataset_avg, dataset_max, initial_train_size, prob_cut_off, update_freq, job_length, cpu_required, cond.var, granularity) {
  ...
  for (ts_num in 1:ncol(dataset_avg)) {
    converted_data <- convert_frequency_dataset(dataset_avg[(initial_train_size+1):nrow(dataset_avg), ts_num], window_size, 'avg')
    test_set_avg <- cbind(test_set_avg, converted_data)
    converted_data <- convert_frequency_dataset(dataset_max[(initial_train_size+1):nrow(dataset_max), ts_num], window_size, 'max')
