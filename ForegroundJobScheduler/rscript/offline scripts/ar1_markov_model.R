...
do_prediction_markov <- function(predictor, transition, predict_size, level=NULL) {
  ...
  prob <- ifelse(is.null(level), NULL, sum(final_transition[from, to:(nrow(transition))]))
...
ar1_markov_model <- function(dataset_avg, dataset_max, initial_train_size, prob_cut_off, max_run_length, window_size, cpu_required, num_of_states, granularity, adjustment) {
  ...
  result_foreground <- sapply(1:length(ts_names), scheduling_foreground, test_dataset_max, test_dataset_avg, coeffs, means, vars, train_result_markov, window_size, prob_cut_off, cpu_required, granularity, schedule_policy)
  
  print("Testing on Model:")
  result_model <- sapply(1:length(ts_names), scheduling_model, test_dataset_max, test_dataset_avg, coeffs, means, vars, train_result_markov, window_size, prob_cut_off, cpu_required, granularity, max_run_length, schedule_policy, adjustment, simplify=FALSE)
...
wrapper.epoche <- function(parameter, dataset_avg, dataset_max, cpu_required, initial_train_size, max_run_length, num_of_states, output_dp, schedule_policy, adjustment) {
