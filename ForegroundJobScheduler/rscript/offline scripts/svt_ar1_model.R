...
wrapper.epoche <- function(parameter, dataset, cpu_required, initial_train_size, max_run_length, output_dp, schedule_policy, adjustment, sample_size, write_result, write_result_path=NULL) {
  ...
  print(system.time(output <- svt_stationary_model(dataset, initial_train_size, window_size, prob_cut_off, max_run_length, cpu_required, granularity, "max", schedule_policy, adjustment)))
  ...
  if (write_result == TRUE) {
    ...
    result_file_name <- paste("AR1", schedule_policy, adjustment, 0, prob_cut_off, granularity, window_size, 0)
