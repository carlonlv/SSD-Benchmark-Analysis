...
ar_logistic_model <- function(dataset_avg, dataset_max, initial_train_size, prob_cut_off, max_run_length, window_size, cpu_required, granularity, cond.var, bin_num, schedule_policy, adjustment) {
...
wrapper.epoche <- function(parameter, dataset_avg, dataset_max, cpu_required, initial_train_size, max_run_length, cond.var, output_dp, schedule_policy, adjustment, sample_size, write_result, write_result_path) {
  ...
  print(system.time(output <- ar_logistic_model(dataset_avg, dataset_max, initial_train_size, prob_cut_off, max_run_length, window_size, cpu_required, granularity, cond.var, bin_num, schedule_policy, adjustment)))
  ...
  if (cond.var == "lm") {
    if (write_result == TRUE) {
      ...
      result_file_name <- paste("AR1_logistic_lm", schedule_policy, adjustment, 0, prob_cut_off, granularity, window_size, bin_num)
    ...
    if (write_result == TRUE) {
      ...
      result_file_name <- paste("AR1_logistic_glm", schedule_policy, adjustment, 0, prob_cut_off, granularity, window_size, bin_num)
      write.csv(ts_results, file = paste0(write_result_path, result_file_name, ".csv"), row.names = TRUE)
    }
