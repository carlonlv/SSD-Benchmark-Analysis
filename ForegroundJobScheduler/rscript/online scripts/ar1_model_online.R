...
wrapper.epoche <- function(parameter, dataset, cpu_required, output_dp, schedule_policy, adjustment) {
  ...
  result_path.csv <- update.df.online(result_path.csv, "AR1", prob_cut_off, 0, sample_size, window_size, granularity, 0, utilization_rate1, utilization_rate2, survival_rate, correct_scheduled_rate, correct_unscheduled_rate)
