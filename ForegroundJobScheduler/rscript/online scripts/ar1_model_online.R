...
wrapper.epoche <- function(parameter, dataset, cpu_required, output_dp, schedule_policy, sample_size, write_result, write_result_path, adjustment) {
  ...
  print(system.time(output <- svt_stationary_model(dataset, train_size, window_size, update_freq, prob_cut_off, cpu_required, granularity, schedule_policy, adjustment)))
  ...
  if (write_result == TRUE) {
    ...
    result_file_name <- paste("AR1", schedule_policy, adjustment, 0, prob_cut_off, granularity, window_size, 0, train_size, update_freq)
