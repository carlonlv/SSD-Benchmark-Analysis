...
svt_model <- function(ts_num, dataset, train_size, window_size, update_freq, prob_cut_off, cpu_required, granularity, schedule_policy="disjoint", num_of_states, adjustment) {
...
svt_stationary_model <- function(dataset, train_size, window_size, update_freq, prob_cut_off, cpu_required, granularity, schedule_policy="disjoint", num_of_states, adjustment) {
  ...
  result <- mclapply(1:length(ts_names), svt_model, dataset, train_size, window_size, update_freq, prob_cut_off, cpu_required, granularity, schedule_policy, num_of_states, adjustment, mc.cores=cores)
...
wrapper.epoche <- function(parameter, dataset, cpu_required, output_dp, schedule_policy, sample_size, write_result, write_result_path, adjustment) {
  ...
  print(system.time(output <- svt_stationary_model(dataset, train_size, window_size, update_freq, prob_cut_off, cpu_required, granularity, schedule_policy, num_of_states, adjustment)))
  ...
  if (write_result == TRUE) {
    ...
    result_file_name <- paste("Markov", schedule_policy, adjustment, num_of_states, prob_cut_off, granularity, window_size, 0, train_size, update_freq)
