...
def check_constraints(data_frame):
    try:
        first_index = data_frame['timestamp'].values[0]
    except:
        first_index = data_frame.index.values[0]
    if len(data_frame) < 8032 or (
            statistics.mean(data_frame['max_cpu']) <= 10 and statistics.mean(data_frame['avg_cpu']) <= 8) or \
            int(first_index) != 0:
        return False
    else:
        return True
...
if __name__ == '__main__':
    ...
    sampled_background = pd.DataFrame(
        np.random.choice(bg_jobs_pool_satisfying_constraints, number_of_bg_job_sampled, replace=False))
